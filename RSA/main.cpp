...
//	RsaKey key2((char*)"1.txt", RSA_PRIVATE_KEY);
...
int main(int argc, char** argv)
{
	...
	if (argc == 5)
	{
		...
		if (!strcmp(argv[1], "decrypt"))
		{
			...
			out_data = RsaCrypt::Decrypt(in_data, data_length, key, &decrypted_length);

			//write to file
			FILE* f_out = fopen(output_file_path, "w");
			fwrite(out_data, 1, decrypted_length, f_out);
			fclose(f_out);
